...
#include "ultralcd.h"
#if ENABLED(ULTRA_LCD_WANHAO_ONEPLUS)
#include "Marlin.h"
#include "language.h"
...
uint8_t lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW; // Set when the LCD needs to draw, decrements after every draw. Set to 2 in LCD routines so the LCD gets at least 1 full redraw (first redraw is partial)
  ...
  void levelCompleteFun();
#define menu_action_gotoScreen(func,encoderPosition,ifRedraw) lcd_goto_screen(func, encoderPosition, ifRedraw)
  void _menu_action_back();
  void _menu_action_levelComplete();
  void menu_action_submenu(screenFunc_t data, bool ifRedraw = true);
  void menu_action_gcode(const char* pgcode);
  void menu_action_function(screenFunc_t data);
  void menu_action_functionEx(screenFunc_t screen, screenFunc_t func);
...
#define MY_MENU_ITEM(TYPE, LABEL, offset,...) do { \
   ...
   */
  void lcd_goto_screen(screenFunc_t screen, const uint32_t encoder = 0, bool ifRedraw = true) {
    if (currentScreen != screen) {
      ...
      #endif
		ifRedraw ? lcdDrawUpdate = LCDVIEW_CALL_REDRAW_NEXT : lcdDrawUpdate = LCDVIEW_NONE;
      screen_changed = true;
      #if ENABLED(DOGLCD)
...
void kill_screen(const char* lcd_msg) {
  ...
  void lcd_main_menu()
  {
	  ...
	  if (card.cardOK)
	  {
		  if (card.isFileOpen())//当处于正在打印时，只显示下面几项选项
		  {
			  MENU_ITEM(setting_edit_int3, MSG_SPEED, PSTR(MSG_SPEED), &feedrate_percentage, 10, 999);
				#if TEMP_SENSOR_0 != 0
				MENU_MULTIPLIER_ITEM(setting_edit_callback_int3, MSG_NOZZLE, PSTR(MSG_NOZZLE), &thermalManager.target_temperature[0], 0, HEATER_0_MAXTEMP - 15, watch_temp_callback_E0);
				#endif
				if (card.sdprinting)
				{
					MENU_ITEM(function, MSG_PAUSE_PRINT, lcd_sdcard_pause);
		  ...
		  {
			  MENU_ITEM(submenu, MSG_PRINTFILE, lcd_sdcard_menu);
			  #if !(PIN_EXISTS(SD_DETECT)||defined(SD_DETECT_BYMYSELF))
			  //MENU_ITEM(gcode, MSG_CNG_SDCARD, PSTR("M21"));  // SD-card changed by user
			  MENU_ITEM(function, MSG_CNG_SDCARD, lcd_sdInit);
			  #endif
		  }
	  }
	  else //SD卡初始失败，提示插卡
	  {
		  ...
		  MENU_ITEM(function, "Insert SD card", lcd_sdInit);
        #endif
	  }
	  ...
	  {
		#if ENABLED(MESH_BED_LEVELING)
		MENU_ITEM(submenu, MSG_LEVEL_BED, my_lcd_level_bed);
		#endif

		MENU_ITEM(submenu, MSG_QUICK_SETTING, myQuickSetting);

		MENU_ITEM(submenu, MSG_ADVANCED, myAdvanced);
	  }
  ...
  void myLcdPreheat()
  {
	  thermalManager.setTargetHotend(min(heater_maxtemp[0], lcd_preheat_hotend_temp[0]), 0);
	  lcd_return_to_status();
  }
  ...
  void lcd_cooldown() {
	...
	*/
	...
	void myLevelGotoNext();
	void lcdLevelReady1();

    // Utility to go to the next mesh point
    // A raise is added between points if Z_HOMING_HEIGHT is in use
    // Note: During Manual Bed Leveling the homed Z position is LCD_PROBE_Z_RANGE
    ...
    void myLevelComplete() {
		START_MENU();
		MY_MENU_ITEM(function, "Back to Main Menu", 2, levelCompleteFun);
		MY_MENU_ITEM(gotoScreen, "Redo Level Wizard", 3, lcdLevelReady1, 0, true);
		END_MENU();
		if (lcdDrawUpdate)
		{
			u8g.setColorIndex(1);
			u8g.setPrintPos(0, 12 * 1);
			lcd_print("Level corner ");
			int step = (GRID_MAX_POINTS_X)* (GRID_MAX_POINTS_Y);
			if (step < 10)lcd_print(step + '0');
			else
			{
				lcd_print(step / 10 + '0');
				lcd_print(step % 10 + '0');
			...
			lcd_print("Levelling Complete");
		}
    }
	void myLevelInterface()
	{
		START_MENU();
		MENU_BACK(MSG_BACK);
		MY_MENU_ITEM(gotoScreen, MSG_GOTO_NEXT_CORNER, 3, myLevelGotoNext, encoderPosition, true);
		END_MENU();
		if (lcdDrawUpdate)
		{
			u8g.setColorIndex(1);
			u8g.setPrintPos(0, 12 * 2);
			lcd_print("Level the corner.");
			u8g.setPrintPos(0, 12 * 3);
			int step = _lcd_level_bed_position;
			if (step < 10)lcd_print(step + '0');
			else
			{
				lcd_print(step / 10 + '0');
			...
			lcd_print('/');
			step = (GRID_MAX_POINTS_X)* (GRID_MAX_POINTS_Y);
			if (step < 10)lcd_print(step + '0');
			else
			{
				lcd_print(step / 10 + '0');
	...
	void myLevelGotoNext() {
		...
		if (_lcd_level_bed_position == (GRID_MAX_POINTS_X)* (GRID_MAX_POINTS_Y))
			lcd_goto_screen(myLevelComplete);
		else
			lcd_goto_screen(myLevelInterface, encoderPosition);
		int8_t px, py;
		mbl.zigzag(_lcd_level_bed_position-1, px, py);
		myMoveToXy(LOGICAL_X_POSITION(mbl.index_to_xpos[px]),
			LOGICAL_Y_POSITION(mbl.index_to_ypos[py]));
		mbl.set_zigzag_z(_lcd_level_bed_position-1, current_position[Z_AXIS]);
	}
	static void lcdLevelReady2()
	{
		_lcd_level_bed_position = 0;
		START_MENU();
		MENU_BACK(MSG_BACK);
		MY_MENU_ITEM(gotoScreen, MSG_NEXT, 4, myLevelGotoNext, encoderPosition, true);
		END_MENU();
		if (lcdDrawUpdate)
		{
			u8g.setColorIndex(1);
			u8g.setPrintPos(0, 12 * 2);
			lcd_print("Turn thumbwheel until");
			u8g.setPrintPos(0, 12 * 3);
			lcd_print("paper can slide betw-");
			u8g.setPrintPos(0, 12 * 4);
			lcd_print("een bed and extruder");
		}
	}
	static void lcdLevelReady1() {
		START_MENU();
		MENU_BACK(MSG_BACK);
		MY_MENU_ITEM(gotoScreen, MSG_NEXT, 4, lcdLevelReady2, encoderPosition, true);
		END_MENU();
		if (lcdDrawUpdate)
		{
			u8g.setColorIndex(1);
			u8g.setPrintPos(0, 12 * 2);
			lcd_print("The level wizard will");
			u8g.setPrintPos(0, 12 * 3);
			lcd_print("guide you through");
			u8g.setPrintPos(0, 12 * 4);
			lcd_print("levelling the bed.");
		}
    }
	...
	static void _lcd_level_bed_homing() {
      ...
      if (axis_homed[X_AXIS] && axis_homed[Y_AXIS] && axis_homed[Z_AXIS])
		  lcd_goto_screen(lcdLevelReady1);
    }
	...
	static u8 filamentStatus;//0位，1进料0退料；1位，1需执行预热
	static int moveDistance = 0;
	void myMoveFilament()
	{
		lcd_implementation_drawmenu_generic(true, 0, PSTR(MSG_BACK), ' ', ' ');
		u8g.setColorIndex(1);
		if (lcd_clicked)
		{
			lcd_return_to_status();
			thermalManager.setTargetHotend(0, E_AXIS);
			planner.discard_current_block();
			if (!(filamentStatus & 0x01))enable_E0();
			return;
		}
		if (filamentStatus & 0x02 && !planner.blocks_queued())//需要加热，则执行加热，清除加热标志
		{
			lcd_preheat_material1_hotend0();
			filamentStatus &= 0xFD;
		}
		if (thermalManager.degHotend(E_AXIS) <= lcd_preheat_hotend_temp[0] - 5)
		{
			u8g.setPrintPos(1, 24);
			lcd_print("Preheating Extruder");
			u8g.setPrintPos(1, 36);
			lcd_print("Please wait...");
			u8g.setPrintPos(1, 48);
			lcd_print(itostr3(thermalManager.current_temperature[active_extruder] + 0.5));
			lcd_print('/');
			lcd_print(itostr3(thermalManager.degTargetHotend(active_extruder) + 0.5));
		}
		else
		{
			if (filamentStatus & 0x01)
			{
				u8g.setPrintPos(1, 24);
				lcd_print("1 Hold filament clamp");
				u8g.setPrintPos(1, 36);
				lcd_print("2 Feed through");
				u8g.setPrintPos(1, 48);
				lcd_print("3 Stop when extrudes");
			}
			else
			{
				u8g.setPrintPos(1, 24);
				lcd_print("1 Hold filament clamp");
				u8g.setPrintPos(1, 36);
				lcd_print("2 Push until extrudes");
				u8g.setPrintPos(1, 48);
				lcd_print("3 Pull out remove");
			}
			if (filamentStatus & 0x01)
			{
				if (!planner.blocks_queued() && commands_in_queue == 0) {
					if (moveDistance < MY_RETRACT_LENGTH)
					{
						int t;
						if (MY_RETRACT_LENGTH>moveDistance + RETRACT_PER_FLASH)
						{
							t = RETRACT_PER_FLASH;
							moveDistance += RETRACT_PER_FLASH;
						}
						else
						{
							t = MY_RETRACT_LENGTH - moveDistance;
							moveDistance = MY_RETRACT_LENGTH;
						}
						//if (filamentStatus & 0x01)current_position[E_AXIS] += t;
						//else current_position[E_AXIS] -= t;
						current_position[E_AXIS] += t;
						planner.buffer_line_kinematic(current_position, MMM_TO_MMS(manual_feedrate_mm_m[E_AXIS]), manual_move_e_index);
					}
					else
					{
						lcd_return_to_status();
			...
			else disable_E0();//201708客户要求，退料解锁电机
		}
		lcdDrawUpdate = LCDVIEW_CALL_REDRAW_NEXT;
	}
	void myRemoveFilament()
	{
		moveDistance = 0;
		filamentStatus = 0x02;//当上次进料退料正在预热中断时，这个标记是true，再次进去后，不会预热，所以，这里要恢复
		enqueue_and_echo_commands_P(PSTR("G28 Z"));
		enqueue_and_echo_commands_P(PSTR("G0 Z50"));
		lcd_goto_screen(myMoveFilament);
	}
	void myAddFilament()
	{
		moveDistance = 0;
		filamentStatus = 0x03;//当上次进料退料正在预热中断时，这个标记是true，再次进去后，不会预热，所以，这里要恢复
		enqueue_and_echo_commands_P(PSTR("G28 Z"));
		enqueue_and_echo_commands_P(PSTR("G0 Z50"));
		lcd_goto_screen(myMoveFilament);
	}
	void myRaiseExtruder10mm()
	{
		current_position[currentAxis] += 10.0;
		manual_move_to_current(currentAxis);
	}
	void myChangeFilament(){
		move_menu_scale = 1;
		START_MENU();
		MENU_BACK(MSG_BACK);
		MY_MENU_ITEM(function, "Add Filament", 1, myAddFilament);
		MY_MENU_ITEM(function, "Remove Filament", 2, myRemoveFilament);
		#if TEMP_SENSOR_0 != 0
		  #if TEMP_SENSOR_1 != 0 || TEMP_SENSOR_2 != 0 || TEMP_SENSOR_3 != 0 || TEMP_SENSOR_BED != 0
			MENU_ITEM(submenu, MSG_PREHEAT_1, lcd_preheat_material1_menu);
		  #else
			MENU_ITEM(function, "Preheat", myLcdPreheat);
		  #endif
		#endif
#if 0
		MENU_ITEM(submenu, "Raise Extruder", _lcd_move_xyz);
#else
		MENU_ITEM(function, "Raise Extruder 10mm", myRaiseExtruder10mm);
#endif
		END_MENU();
	}
	...
	void myQuickSetting(){
		...
		START_MENU();
		MENU_BACK(MSG_BACK);
		MENU_ITEM(submenu, "Move", myMoveAxis);
		MENU_ITEM(submenu, "Change Filament", myChangeFilament);
		MENU_ITEM(gcode, MSG_DISABLE_STEPPERS, PSTR("M84"));
		#if TEMP_SENSOR_0 != 0
		  #if TEMP_SENSOR_1 != 0 || TEMP_SENSOR_2 != 0 || TEMP_SENSOR_3 != 0 || TEMP_SENSOR_BED != 0
			MENU_ITEM(submenu, "Preheat", lcd_preheat_material1_menu);
		  #else
			MENU_ITEM(function, "Preheat", myLcdPreheat);
		  #endif
		#endif
	...
	void myFactoryReset()
	{
		START_MENU();
		MENU_BACK(MSG_BACK);
		MY_MENU_ITEM(function, "Yes", 3, myResetAndReturn);
		END_MENU();
		if (lcdDrawUpdate)
		{
			u8g.setColorIndex(1);
			u8g.setPrintPos(1, 24);
			lcd_print("Restore factory setti");
			u8g.setPrintPos(1, 36);
			lcd_print("ngs, are you sure ? ");
		}
	}
	void myAdvanced(){
		START_MENU();
		...
		//MENU_ITEM(submenu, "Advanced Motion", lcd_control_motion_menu);//201708客户要求去掉这一条

		#if ENABLED(EEPROM_SETTINGS)
		MENU_ITEM(function, "Save Setting", settings.save);
		#endif
		  MENU_ITEM(submenu, "Factory Reset", myFactoryReset);
		END_MENU();
	}
  ...
  void _lcd_move_xyz() {
    ...
    if (encoderPosition) {
	  ...
	  float min = soft_endstops_enabled ? soft_endstop_min[currentAxis] : current_position[currentAxis] - 1000,
#else
	  float min = current_position[currentAxis] - 1000,
#endif
#if ENABLED(MAX_SOFTWARE_ENDSTOPS)
		  max = soft_endstops_enabled ? soft_endstop_max[currentAxis] : current_position[currentAxis] + 1000;
#else
		  max = current_position[currentAxis] + 1000;
#endif

      // Get the new position
      current_position[currentAxis] += float((int32_t)encoderPosition) * move_menu_scale;

      // Delta limits XY based on the current offset from center
      // This assumes the center is 0,0
      #if ENABLED(DELTA)
        if (currentAxis != Z_currentAxis) {
          max = sqrt(sq(DELTA_PRINTABLE_RADIUS) - sq(current_position[Y_currentAxis - currentAxis]));
          min = -max;
        }
      #endif

      // Limit only when trying to move towards the limit
      if ((int32_t)encoderPosition < 0) NOLESS(current_position[currentAxis], min);
      if ((int32_t)encoderPosition > 0) NOMORE(current_position[currentAxis], max);
	...
	if (lcdDrawUpdate)
	{
		...
		u8g.drawBox(0, 15, LCD_PIXEL_WIDTH, DOG_CHAR_HEIGHT - 1 + 2 * (TALL_FONT_CORRECTION));
		u8g.setColorIndex(0); // white on black
		u8g.setPrintPos(1, 24);
		lcd_print(MSG_BACK);
		u8g.setColorIndex(1);
		u8g.setPrintPos(1, 36);
		lcd_print("Turn dial to move");
		u8g.setPrintPos(1, 48);
		lcd_print("Move:");
		lcd_print(ftostr31(current_position[currentAxis]));
	}
  }
  void _lcd_move_e(
    #if E_MANUAL > 1
    ...
    if (lcdDrawUpdate) {
      ...
      #else
        switch (eindex) {
          default: pos_label = MSG_MOVE_E MSG_MOVE_E1; break;
          case 1: pos_label = MSG_MOVE_E MSG_MOVE_E2; break;
          #if E_MANUAL > 2
            case 2: pos_label = MSG_MOVE_E MSG_MOVE_E3; break;
            #if E_MANUAL > 3
              case 3: pos_label = MSG_MOVE_E MSG_MOVE_E4; break;
            #endif
          #endif
        }
      #endif
	  u8g.setColorIndex(1); // black on white
	  u8g.drawBox(0, 5, LCD_PIXEL_WIDTH, DOG_CHAR_HEIGHT - 1 + 2 * (TALL_FONT_CORRECTION));
	  u8g.setColorIndex(0); // white on black
	  ...
	  u8g.setPrintPos(1, 48);
	  lcd_print("Move:");
	  lcd_print(ftostr31(current_position[E_AXIS]));
    }
  }

  void lcd_move_menu_10mm() {
	  move_menu_scale = 10.0;
	  if (currentAxis == X_AXIS || currentAxis == Y_AXIS || currentAxis == Z_AXIS )
		  _lcd_move_xyz();
	  else
		  _lcd_move_e(
#if E_MANUAL > 1
  ...
  void myMoveAxis() {
    ...
    #endif
    #endif
		MENU_ITEM(gcode, "Home XYZ", PSTR("G28"));
  ...
  void lcd_control_temperature_menu() {
    ...
    //
      ...
      //MENU_ITEM_EDIT(bool, MSG_AUTOTEMP, &planner.autotemp_enabled);//201708客户要求去掉这一条
  ...
  void _menu_action_levelComplete()
  {
	  lcd_goto_screen(myLcdLevelDone);
	  current_position[Z_AXIS] = LCD_PROBE_Z_RANGE + Z_HOMING_HEIGHT;
	  line_to_current(Z_AXIS);
	  stepper.synchronize();
  ...
  void menu_action_submenu(screenFunc_t func, bool ifRedraw) { lcd_save_previous_screen(); lcd_goto_screen(func, 0,ifRedraw); }
...
void lcd_update() {
  ...
  if (ELAPSED(ms, next_lcd_update_ms)) {
    ...
    if (currentScreen == lcd_status_screen && !(++lcd_status_update_delay % 10))
      lcdDrawUpdate = LCDVIEW_REDRAW_NOW;
	...
	if (currentScreen == lcd_status_screen || currentScreen == myMoveFilament || defer_return_to_status)//当处于进料退料时，不回到主界面
        return_to_status_ms = ms + LCD_TIMEOUT_TO_STATUS;
...
#endif // ULTRA_LCD_WANHAO_ONEPLUS
