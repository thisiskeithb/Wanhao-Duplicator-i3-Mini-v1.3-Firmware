...
const char PortArrayLen = sizeof(PortArray) / sizeof(PortArray[0]);
static char serialIndex = 0;
#endif
...
inline void get_serial_commands() {
...
#ifdef USED_SERIAL_MAX
  {
	  char i;
	  //char j = serialIndex;
	  for (i = 0; i < PortArrayLen; i++, serialIndex++)//每一次都只读取一个serialIndex指向的口
	  {
		  if (serialIndex >= PortArrayLen)serialIndex = 0;
		  MYSERIAL.setPort(_BV(PortArray[serialIndex]));
		  if (MYSERIAL.available()>0)break;
	  }
	  if (i == PortArrayLen)//当所有串口都没有命令时，将串口指向原处
	  {
		  //MYSERIAL.setPort(_BV(PortArray[j));
		  MYSERIAL.setPort(0xFF);//YSZ-GOGO 当前的串口返回是广播式的，即一个串口请求数据，全部都返回
		  return;
	  }
  }
#endif
  ...
  for(char i=0;serial_line_buffer[i];i++) MYSERIAL.write(serial_line_buffer[i]);
  #endif
#endif
}
...
static void clean_up_after_endstop_or_probe_move() {
  ...
  bool set_probe_deployed(bool deploy) {
    ...
    // Make room for probe
#ifndef UNNEED_MAKE_ROOM_FOR_PROBE
...
inline void gcode_G28(const bool always_home_all) {
  ...
  endstops.enable(true); // Enable endstops for next homing move
    ...
    set_destination_to_current();
#if CLIENT_VAR==WANHAO_ONEPLUS_201707VAR//复位时，Z轴至少要保证一个高度，大小是Z_HOMING_HEIGHT，如果当前高度不够，会抬升
	if (!myNeedRetPos)
	{
#endif
...
inline void gcode_M109() {
  ...
  if (wait_for_heatup)
  {
	  if (card.sdprinting)LCD_MESSAGEPGM(MSG_PRINTING);
	  else LCD_MESSAGEPGM(MSG_HEATING_COMPLETE);
  }
#else //marlin默认
  if (wait_for_heatup) {
...
#endif
#if ENABLED(SDSUPPORT)
  card.checkautostart(false);//YSZ-WORK
#endif
