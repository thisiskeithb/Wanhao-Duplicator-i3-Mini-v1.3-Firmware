...
static void abortPrint()
{
    ...
    //card.pause = false; //YSZ-GOGO
    pauseRequested = false;

    _enqueuecommand("M401");
    ...
    if (primed)
    {
        ...
        sprintf_P(buffer, "G92 E%i", int(((float)END_OF_PRINT_RETRACTION) / volumetric_multiplier[active_extruder]));
        _enqueuecommand(buffer);
        // perform the retraction at the standard retract speed
		sprintf_P(buffer, "G1 F%i E0", int(retract_feedrate_mm_s));
        _enqueuecommand(buffer);
	#endif
#else
		// set up the end of print retraction
...
static void doStartPrint()
{
    ...
    {	
        ...
        if (!primed)
        {
			...
			planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], homing_feedrate_mm_s[Z_AXIS], active_extruder);
            // note that we have primed, so that we know to de-prime at the end
            primed = true;
        }

        // undo the end-of-print retraction
        planner.set_e_position_mm((0.0 - END_OF_PRINT_RETRACTION) / volumetric_multiplier[e]);
		...
		planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], (PRIMING_MM3_PER_SEC * volumetric_multiplier[e]), e);
	#if EXTRUDERS > 1
		// for extruders other than the first one, perform end of print retraction
		if (e > 0)
		{
			planner.set_e_position_mm((END_OF_PRINT_RETRACTION) / volumetric_multiplier[e]);
		...
		if (e > 0)
		{
			plan_set_e_position((END_OF_PRINT_RETRACTION) / volumetric_multiplier[e]);
        ...
        if (selIndex == 0)
        {
            ...
            if (!card.filenameIsDir)
            {
                ...
                if (card.isFileOpen() && !is_command_queued())
                {
                    ...
                    card.setIndex(0);
#if FAN_COUNT > 0
					for (uint8_t e = 0; e<FAN_COUNT; e++)fanSpeeds[e] = 0;
#endif
					//fanSpeed = 0;
                    ...
                    if (strcmp_P(buffer, PSTR(";FLAVOR:UltiGCode")) == 0)
                    {
                        ...
                        for(uint8_t e=0; e<EXTRUDERS; e++)
                        {
                            if (LCD_DETAIL_CACHE_MATERIAL(e) < 1)
                                continue;
#if 1
							Temperature::target_temperature[e] = 0;//material[e].temperature;
				#if TEMP_SENSOR_BED != 0
							Temperature::target_temperature_bed = max(Temperature::target_temperature_bed, material[e].bed_temperature);
				#endif
#else
				...
				#endif
#endif
                            volumetric_multiplier[e] = 1.0 / (M_PI * (material[e].diameter / 2.0) * (material[e].diameter / 2.0));
							flow_percentage[e] = material[e].flow;
                            //extrudemultiply[e] = material[e].flow;
                        }

                        _enqueuecommand("G28");
                        _enqueuecommand(HEATUP_POSITION_COMMAND);
                        ...
                        for(uint8_t e=0; e<EXTRUDERS; e++)
                        {
                            volumetric_multiplier[e] = 1.0;
							flow_percentage[e] = 100;
...
static void lcd_menu_print_heatup()
{
...
#if TEMP_SENSOR_BED != 0
	if (Temperature::current_temperature_bed > Temperature::target_temperature_bed - 10)
    //if (current_temperature_bed > target_temperature_bed - 10)
    {
#endif
        for(uint8_t e=0; e<EXTRUDERS; e++)
        {
			if (LCD_DETAIL_CACHE_MATERIAL(e) < 1 || Temperature::target_temperature[e] > 0)
            //if (LCD_DETAIL_CACHE_MATERIAL(e) < 1 || target_temperature[e] > 0)
                continue;
			Temperature::target_temperature[e] = material[e].temperature;
            //target_temperature[e] = material[e].temperature;
        }
#if 1
		if (Temperature::current_temperature_bed >= Temperature::target_temperature_bed - TEMP_WINDOW * 2 && !is_command_queued())
		{
			bool ready = true;
			for (uint8_t e = 0; e<EXTRUDERS; e++)
			if (Temperature::current_temperature[e] < Temperature::target_temperature[e] - TEMP_WINDOW)
				ready = false;
#else
    ...
    for(uint8_t e=0; e<EXTRUDERS; e++)
    {
		if (LCD_DETAIL_CACHE_MATERIAL(e) < 1 || Temperature::target_temperature[e] < 1)
		//if (LCD_DETAIL_CACHE_MATERIAL(e) < 1 || target_temperature[e] < 1)
            continue;
#if 1
		if (Temperature::current_temperature[e] > 20)
			progress = min(progress, (Temperature::current_temperature[e] - 20) * 125 / (Temperature::target_temperature[e] - 20 - TEMP_WINDOW));
#else
...
#if TEMP_SENSOR_BED != 0
#if 1
	if (Temperature::current_temperature_bed > 20)
		progress = min(progress, (Temperature::current_temperature_bed - 20) * 125 / (Temperature::target_temperature_bed - 20 - TEMP_WINDOW));
	else if (Temperature::target_temperature_bed > Temperature::current_temperature_bed - 20)
#else
...
static void lcd_menu_print_printing()
{
	...
	if (!card.sdprinting)
    {
        ...
        if (lcd_lib_button_pressed)
        {
            if (IS_SELECTED_MAIN(0) && Planner::movesplanned() < 1)
            {
                //card.pause = false;
                //if (card.sdprinting)
                {
                ...
                lcd_lib_beep();
            }else if (IS_SELECTED_MAIN(1) && Planner::movesplanned() < 1)
    ...
    else
    {
        ...
        case PRINT_STATE_HEATING:
            ...
            *c++ = '/';
            c = int_to_string(Temperature::target_temperature[0], c, PSTR("C"));
        ...
        case PRINT_STATE_HEATING_BED:
            ...
            *c++ = '/';
			c = int_to_string(Temperature::target_temperature_bed, c, PSTR("C"));
			//c = int_to_string(target_temperature_bed, c, PSTR("C"));
            lcd_lib_draw_string_center(30, buffer);
            break;
		}
		float printTimeMs = print_job_timer.duration();
...
static void lcd_menu_print_ready()
{
	...
	if (Temperature::current_temperature[0] > 60 || Temperature::current_temperature_bed > 40)
	//if (current_temperature[0] > 60 || current_temperature_bed > 40)
    {
        lcd_lib_draw_string_centerP(15, PSTR("Printer cooling down"));

		int16_t progress = 124 - (Temperature::current_temperature[0] - 60);
...
static void tune_item_details_callback(uint8_t nr)
{
    char* c = card.longFilename;
	if (nr == 2)
		c = int_to_string(feedrate_percentage, c, PSTR("%"));
    ...
    else if (nr == 3 + EXTRUDERS)
    {
        ...
        *c++ = '/';
		...
		c = int_to_string(int(fanSpeeds[0]) * 100 / 255, c, PSTR("%"));
#endif
    //else if (nr == 3 + BED_MENU_OFFSET + EXTRUDERS)
        //c = int_to_string(int(fanSpeed) * 100 / 255, c, PSTR("%"));
    else if (nr == 4 + BED_MENU_OFFSET + EXTRUDERS)
		c = int_to_string(flow_percentage[0], c, PSTR("%"));
        //c = int_to_string(extrudemultiply[0], c, PSTR("%"));
#if EXTRUDERS > 1
    else if (nr == 5 + BED_MENU_OFFSET + EXTRUDERS)
		c = int_to_string(flow_percentage[1], c, PSTR("%"));
...
void lcd_menu_print_tune_heatup_nozzle0()
{
    ...
    {
#if 1
		Temperature::target_temperature[0] = int(Temperature::target_temperature[0]) + (lcd_lib_encoder_pos / ENCODER_TICKS_PER_SCROLL_MENU_ITEM);
		if (Temperature::target_temperature[0] < 0)
			Temperature::target_temperature[0] = 0;
		if (Temperature::target_temperature[0] > HEATER_0_MAXTEMP - 15)
			Temperature::target_temperature[0] = HEATER_0_MAXTEMP - 15;
#else
	...
	int_to_string(int(Temperature::target_temperature[0]), buffer + strlen(buffer), PSTR("C"));
...
static void lcd_retraction_details(uint8_t nr)
{
    ...
    if (nr == 0)
		...
		int_to_string(retract_feedrate_mm_s, buffer, PSTR("mm/sec"));
...
static void lcd_menu_print_tune_retraction()
{
    ...
    if (lcd_lib_button_pressed)
    {
        if (IS_SELECTED_SCROLL(0))
			...
			LCD_EDIT_SETTING_SPEED(retract_feedrate_mm_s, "Retract speed", "mm/sec", 0, planner.max_feedrate_mm_s[E_AXIS] * 60);
...
static void lcd_menu_print_pause()
{
    ...
    {
		if (Planner::movesplanned() > 0 && commands_in_queue < BUFSIZE)
        {
			pauseRequested = false;
			card.sdprinting = false;
            //card.pause = true;
