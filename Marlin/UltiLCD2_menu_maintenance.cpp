...
{
	lcd_tripple_menu(PSTR("AUTO BED|LEVELING"), PSTR("ADVANCED"), PSTR("Return"));
...
static char* lcd_advanced_item(uint8_t nr)
{
	if (nr == 0)
		...
		strcpy_P(card.longFilename, PSTR("Mannual bed leveling"));
		//strcpy_P(card.longFilename, PSTR("Auto plate"));
	else
...
static void lcd_advanced_details(uint8_t nr)
{
	...
	else if (nr == 2 + EXTRUDERS)
	{
		...
		int_to_string(int(Temperature::target_temperature_bed), buffer + strlen(buffer), PSTR("C"));
	...
	{
#if FAN_COUNT > 0
		int_to_string(int(fanSpeeds[0]) * 100 / 255, buffer, PSTR("%"));
#endif
...
static void lcd_menu_maintenance_advanced()
{
    lcd_scroll_menu(PSTR("ADVANCED"), 13 + BED_MENU_OFFSET + EXTRUDERS * 2, lcd_advanced_item, lcd_advanced_details);
    if (lcd_lib_button_pressed)
    {
        ...
        else if (IS_SELECTED_SCROLL(6 + BED_MENU_OFFSET + EXTRUDERS))
        {
			...
			active_extruder = 0;
			Temperature::target_temperature[active_extruder] = material[active_extruder].temperature;
...
static void lcd_menu_maintenance_advanced_heatup()
{
    ...
    {
#if 1
        Temperature::target_temperature[active_extruder] = int(Temperature::target_temperature[active_extruder]) + (lcd_lib_encoder_pos / ENCODER_TICKS_PER_SCROLL_MENU_ITEM);
        if (Temperature::target_temperature[active_extruder] < 0)
            Temperature::target_temperature[active_extruder] = 0;
        if (Temperature::target_temperature[active_extruder] > HEATER_0_MAXTEMP - 15)
            Temperature::target_temperature[active_extruder] = HEATER_0_MAXTEMP - 15;
#else
    ...
    int_to_string(int(Temperature::target_temperature[active_extruder]), buffer+strlen(buffer), PSTR("C"));
...
void lcd_menu_maintenance_extrude()
{
    ...
    {
#if ENABLED(PREVENT_COLD_EXTRUSION)
		Temperature::extrude_min_temp = EXTRUDE_MINTEMP;
#endif
...
void lcd_menu_maintenance_advanced_bed_heatup()
{
    ...
    {
#if 1
        Temperature::target_temperature_bed = int(Temperature::target_temperature_bed) + (lcd_lib_encoder_pos / ENCODER_TICKS_PER_SCROLL_MENU_ITEM);
        if (Temperature::target_temperature_bed < 0)
            Temperature::target_temperature_bed = 0;
        if (Temperature::target_temperature_bed > BED_MAXTEMP - 15)
            Temperature::target_temperature_bed = BED_MAXTEMP - 15;
#else
...
static void lcd_retraction_details(uint8_t nr)
{
    ...
    if (nr == 0)
        ...
        int_to_string(retract_feedrate_mm_s + 0.5, buffer, PSTR("mm/sec"));
...
static void lcd_motion_details(uint8_t nr)
{
    ...
    if (nr == 0)
        ...
        int_to_string(planner.acceleration, buffer, PSTR("mm/sec^2"));
    else if(nr == 2)
		float_to_string(planner.max_jerk[X_AXIS], buffer, PSTR("mm/sec"));
    else if(nr == 3)
        int_to_string(planner.max_feedrate_mm_s[X_AXIS], buffer, PSTR("mm/sec"));
    else if(nr == 4)
        int_to_string(planner.max_feedrate_mm_s[Y_AXIS], buffer, PSTR("mm/sec"));
    else if(nr == 5)
        int_to_string(planner.max_feedrate_mm_s[Z_AXIS], buffer, PSTR("mm/sec"));
	#if HAS_MOTOR_CURRENT_PWM
    else if(nr == 6)
        int_to_string(Stepper::motor_current_setting[0], buffer, PSTR("mA"));
    else if(nr == 7)
        int_to_string(Stepper::motor_current_setting[1], buffer, PSTR("mA"));
    else if(nr == 8)
        int_to_string(Stepper::motor_current_setting[2], buffer, PSTR("mA"));
	#endif
#else
	if (nr == 0)
		return;
...
static void lcd_menu_maintenance_motion()
{
    ...
    if (lcd_lib_button_pressed)
    {
        ...
        if (IS_SELECTED_SCROLL(0))
        {
            ...
            LCD_EDIT_SETTING_FLOAT100(planner.acceleration, "Acceleration", "mm/sec^2", 0, 20000);
        else if (IS_SELECTED_SCROLL(2))
			LCD_EDIT_SETTING_FLOAT1(planner.max_jerk[X_AXIS], "X/Y Jerk", "mm/sec", 0, 100);
        else if (IS_SELECTED_SCROLL(3))
            LCD_EDIT_SETTING_FLOAT1(planner.max_feedrate_mm_s[X_AXIS], "Max speed X", "mm/sec", 0, 1000);
        else if (IS_SELECTED_SCROLL(4))
            LCD_EDIT_SETTING_FLOAT1(planner.max_feedrate_mm_s[Y_AXIS], "Max speed Y", "mm/sec", 0, 1000);
        else if (IS_SELECTED_SCROLL(5))
            LCD_EDIT_SETTING_FLOAT1(planner.max_feedrate_mm_s[Z_AXIS], "Max speed Z", "mm/sec", 0, 1000);
	#if HAS_DIGIPOTSS || HAS_MOTOR_CURRENT_PWM
		else if (IS_SELECTED_SCROLL(6))
			LCD_EDIT_SETTING(Stepper::motor_current_setting[0], "Current X/Y", "mA", 0, 1300);
		else if (IS_SELECTED_SCROLL(7))
			LCD_EDIT_SETTING(Stepper::motor_current_setting[1], "Current Z", "mA", 0, 1300);
		else if (IS_SELECTED_SCROLL(8))
			LCD_EDIT_SETTING(Stepper::motor_current_setting[2], "Current E", "mA", 0, 1300);
	#endif
#else
		if (IS_SELECTED_SCROLL(0))
		{
