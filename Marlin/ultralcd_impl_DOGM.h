...
#if defined(YSZ_TEST_CONFIG)||CLIENT_VAR==WANHAO_ONEPLUS_201707VAR
...
static void lcd_implementation_status_screen() {
  ...
  if (PAGE_UNDER(STATUS_SCREENHEIGHT + 20)) {
	  u8g.drawBitmapP(110, 20, STATUS_SCREENBYTEWIDTH, STATUS_SCREENHEIGHT,
      #if HAS_FAN0
  ...
  //
#if CLIENT_VAR==WANHAO_ONEPLUS_201707VAR
  if (PAGE_CONTAINS(7, 19))
		_draw_centered_temp(thermalManager.target_temperature[0] + 0.5, 104, 19);
	if (PAGE_CONTAINS(33, 40))
		_draw_centered_temp(thermalManager.degHotend(0) + 0.5, 104, 40);

	if (PAGE_CONTAINS(29, 32)) {
		if (thermalManager.isHeatingHotend(0)) {
			u8g.setColorIndex(0); // white on black
			u8g.drawBox(114, 29, 2, 2);
			u8g.setColorIndex(1); // black on white
		}
		else {
			u8g.drawBox(114, 29, 2, 2);
  ...
  u8g.setPrintPos(0, 49);
	u8g.print("Progress");
	// SD Card Symbol
	//SD图标是10像素宽=8+2，11像素高7+4
	u8g.drawBox(2, 52, 8, 7);   //2-9 
	// Right edge
	u8g.drawBox(10, 54, 2, 5);     //10-11
	// Bottom hollow box
	u8g.drawFrame(2, 59, 10, 4);  //2-11
	// Corner pixel
	u8g.drawPixel(10, 53);         //10,51一点
	//
	// Progress bar frame
	//
...
#define PROGRESS_BAR_WIDTH 80
	...
	if (PAGE_CONTAINS(59, 62 - (TALL_FONT_CORRECTION)))       // 49-52 (or 49-51)
		u8g.drawFrame(
		PROGRESS_BAR_X, 59,
	...
	if (IS_SD_PRINTING) {
		...
		if (PAGE_CONTAINS(60, 61 - (TALL_FONT_CORRECTION)))     // 50-51 (or just 50)
			u8g.drawBox(
			PROGRESS_BAR_X + 1, 60,
...
#if CLIENT_VAR==WANHAO_ONEPLUS_201707VAR //客户不需要屏上显示坐标
#else //marlin默认
...
#if CLIENT_VAR==WANHAO_ONEPLUS_201707VAR
#define STATUS_BASELINE INFO_FONT_HEIGHT
#else //marlin默认
