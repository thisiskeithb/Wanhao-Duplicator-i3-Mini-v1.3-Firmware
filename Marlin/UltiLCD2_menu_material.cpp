...
int  PLA_temperature_offset = 200;
int  PLA_bed_temperature_offset = 60;
char PLA_fan_speed_offset = 100;
int	 PLA_flow_offset = 100;
float PLA_diameter_offset = 1.75;

int  ABS_temperature_offset = 240;
int  ABS_bed_temperature_offset = 90;
char ABS_fan_speed_offset = 50;
int	 ABS_flow_offset = 100;
float ABS_diameter_offset = 1.75;

int  CPE_temperature_offset = 250;
int  CPE_bed_temperature_offset = 60;
char CPE_fan_speed_offset = 50;
int	 CPE_flow_offset = 100;
float CPE_diameter_offset = 1.75;
...
static void lcd_menu_material_main()
{
    ...
    if (lcd_lib_button_pressed)
    {
        if (IS_SELECTED_MAIN(0) && !is_command_queued())
        {
            ...
            char buffer[32];
            _enqueuecommand("G28 X0 Y0");
			sprintf_P(buffer, "G1 F%i X%i Y%i", int(homing_feedrate_mm_s[0]), X_MAX_LENGTH / 2, 10);
...
{
#if 1
    Temperature::setTargetHotend(material[active_extruder].temperature, active_extruder);
    int16_t temp = Temperature::degHotend(active_extruder) - 20;
    int16_t target = Temperature::degTargetHotend(active_extruder) - 20 - 10;
#else
    ...
    if (temp > target)
    {
		...
		planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], 20.0 / volumetric_multiplier[active_extruder], retract_feedrate_mm_s / 60.0, active_extruder);
		float old_max_feedrate_e = planner.max_feedrate_mm_s[E_AXIS];
        float old_retract_acceleration = planner.retract_acceleration;
		planner.max_feedrate_mm_s[E_AXIS] = FILAMENT_REVERSAL_SPEED;
		planner.retract_acceleration = FILAMENT_LONG_MOVE_ACCELERATION;
		planner.set_e_position_mm(0);
		planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], -1.0 / volumetric_multiplier[active_extruder], FILAMENT_REVERSAL_SPEED, active_extruder);
		planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], -FILAMENT_REVERSAL_LENGTH / volumetric_multiplier[active_extruder], FILAMENT_REVERSAL_SPEED, active_extruder);
#else
...
static void lcd_menu_change_material_remove()
{
	...
	long targetPos = lround(FILAMENT_REVERSAL_LENGTH * planner.axis_steps_per_mm[E_AXIS]);
#else
	if (!blocks_queued())
	{
		lcd_lib_beep();
...
static void lcd_menu_insert_material_preheat()
{
    ...
    {
		Temperature::extrude_min_temp = 0;
        for(uint8_t e=0; e<EXTRUDERS; e++)
            volumetric_multiplier[e] = 1.0;//Set the extrusion to 1mm per given value, so we can move the filament a set distance.
...
#if 1
    LED_GLOW();
	if (printing_state == PRINT_STATE_NORMAL && Planner::movesplanned() < 2)
    {
        planner.set_e_position_mm(0);
        planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], 0.5 / volumetric_multiplier[active_extruder], FILAMENT_INSERT_SPEED, active_extruder);
    }
#else
...
static void lcd_menu_change_material_insert_wait_user_ready()
{
    ...
    planner.set_e_position_mm(0);
    planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], FILAMENT_FORWARD_LENGTH / volumetric_multiplier[active_extruder], FILAMENT_INSERT_FAST_SPEED, active_extruder);
...
static void lcd_menu_change_material_insert_forward()
{
    ...
    if (!planner.is_full())
    {
        ...
        led_glow_dir = led_glow = 0;
	#if HAS_DIGIPOTSS || HAS_MOTOR_CURRENT_PWM
        stepper.digipot_current(2, Stepper::motor_current_setting[2]*2/3);//Set the E motor power lower to we skip instead of grind.
	#endif
...
{
    planner.set_e_position_mm(0);
    planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], -END_OF_PRINT_RETRACTION / volumetric_multiplier[active_extruder], 25*60, active_extruder);
...
static void lcd_menu_change_material_insert()
{
    ...
    {
        planner.set_e_position_mm(0);
        planner.buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], 0.5 / volumetric_multiplier[active_extruder], FILAMENT_INSERT_EXTRUDE_SPEED, active_extruder);
    }
#else
	if (movesplanned() < 2)
	{
...
void lcd_material_reset_defaults()
{
	...
	eeprom_write_word(EEPROM_MATERIAL_TEMPERATURE_OFFSET(0), PLA_temperature_offset);
	eeprom_write_word(EEPROM_MATERIAL_BED_TEMPERATURE_OFFSET(0), PLA_bed_temperature_offset);
	eeprom_write_byte(EEPROM_MATERIAL_FAN_SPEED_OFFSET(0), PLA_fan_speed_offset);
	eeprom_write_word(EEPROM_MATERIAL_FLOW_OFFSET(0), PLA_flow_offset);
	eeprom_write_float(EEPROM_MATERIAL_DIAMETER_OFFSET(0), PLA_diameter_offset);

	strcpy_P(buffer, PSTR("ABS"));
	eeprom_write_block(buffer, EEPROM_MATERIAL_NAME_OFFSET(1), 4);
	eeprom_write_word(EEPROM_MATERIAL_TEMPERATURE_OFFSET(1), ABS_temperature_offset);
	eeprom_write_word(EEPROM_MATERIAL_BED_TEMPERATURE_OFFSET(1), ABS_bed_temperature_offset);
	eeprom_write_byte(EEPROM_MATERIAL_FAN_SPEED_OFFSET(1), ABS_fan_speed_offset);
	eeprom_write_word(EEPROM_MATERIAL_FLOW_OFFSET(1), ABS_flow_offset);
	eeprom_write_float(EEPROM_MATERIAL_DIAMETER_OFFSET(1), ABS_diameter_offset);

	strcpy_P(buffer, PSTR("CPE"));
	eeprom_write_block(buffer, EEPROM_MATERIAL_NAME_OFFSET(2), 5);
	eeprom_write_word(EEPROM_MATERIAL_TEMPERATURE_OFFSET(2), CPE_temperature_offset);
	eeprom_write_word(EEPROM_MATERIAL_BED_TEMPERATURE_OFFSET(2), CPE_bed_temperature_offset);
	eeprom_write_byte(EEPROM_MATERIAL_FAN_SPEED_OFFSET(2), CPE_fan_speed_offset);
	eeprom_write_word(EEPROM_MATERIAL_FLOW_OFFSET(2), CPE_flow_offset);
	eeprom_write_float(EEPROM_MATERIAL_DIAMETER_OFFSET(2), CPE_diameter_offset);
